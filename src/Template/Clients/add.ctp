<div class="d-none client-id"><?= $id ?></div>
<div class="wrapper wrapper-content animated fadeIn faster ml-2">
	<div class="max-wh ibox">
		<div class="ibox-content">
			...
			<?= $this->Form->create(null, ['class' => 'form d-none','id' => 'form']) ?>
			...
			<div class="societe-group mb-3">
				...
				<hr>
				<?= $this->Form->select('compagnie_id', $this->Custom->getCompagnies() , ['empty' => 'Sélectionner une société', 'class' => 'form-control', 'id' => 'compagnie_id', 'label' => false, 'default' => $id]) ?>
			</div>
			<hr>
			...
			<input type="submit" value="Créer le client" class="btn btn-primary create">
			<?= $this->Form->end() ?>
		</div>
	</div>
</div>
